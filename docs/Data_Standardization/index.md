...
* **Attribute semantics**: As suggested by its coding or plain names, an attribute like "age" can have a **semantic type** different from its **datatype**, so for example while an attribute might simply be called "age" (with datatype integer and implicit year unit), it wasn't intended to be compared with any dataset's "age" attribute out there in the world, as shown by the list of [age kinds](http://purl.obolibrary.org/obo/OBI_0001167) from the OBI ontology. So, while an attribute might state this in its plain language definition, the semantics of that are lost on computers trying to see if two age fields are comparable.  This is why data harmonization proponents advocate to include a mapping to one or more comparable ontology term ids in an attribute specification (or alternately, in a knowledge graph view, to use an ontology term identifier directly to indicate an attribute).
...
* **Categorical attributes**: Some attributes take on a set of permissable values that are coming from one or more controlled vocabularies.  Data schemas need a mechanism to indicate what the vocabulary "picklist" choices are, or, in the case of large and evolving vocabularies, where they are online, and which branches to include or exclude choices from.
