...
* **Attribute semantics**: As suggested by its coding or plain names, an attribute like "age" has a **semantic type** different from its **datatype**, so for example while an attribute might simply be called "age" (with datatype integer and implicit year unit), it wasn't intended to be compared with any dataset's "age" attribute out there in the world, as shown by the list of [age kinds](http://purl.obolibrary.org/obo/OBI_0001167) from the OBI ontology. So, while an attribute might state this in its plain language definition, the semantics of that (setting aside AI tools) are lost on computers trying to see if two age fields are comparable.  This is why data harmonization proponents advocate to include a mapping to one or more comparable ontology term ids in an attribute specification (or alternately, in a knowledge graph view, to use an ontology term identifier directly to indicate an attribute).
  * Another complication is that attributes aren't always measured in the same way, even within the same schema.  To disambiguate attributes having different measures of the same semantic thing, one must include unit information inin a schema attribugte's specificaiton, and convey this in the attribute's coding name ( such as "age_in_years" or "age_in_weeks" or "gestational_age" or "trimester_age").  
